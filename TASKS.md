...
|----|-------|--------|-------|
| T-012 | Re-run T-009 restore against volume-backed dabe DB | 2026-04-28 | **No new merge — closed by verification.** Goal of T-012 was to ensure the T-009 restore data lives on the volume-backed dabe DB rather than the ephemeral `/app/data` from before T-011a. Verified via prod row counts on 2026-04-28: `tools=133`, `services=5` — both exact matches for the T-009 backup baseline (see T-009 row, T-010 row "Wipe #3"). Conclusion: T-011a's volume attach (`9638aa3`) plus subsequent re-population (likely a sync from the legacy 7058a source after the volume came online) implicitly satisfied this task. No code change required. T-012 branch (`task/restore-dabe-from-snapshot`) carried no implementation commits — only the housekeeping commits closing T-011a (`57f4875`) and filing T-013 (`62a8890`) — and was repurposed into the T-015 branch (`task/sync-tasks-notion-mirror`) for shipping. |
