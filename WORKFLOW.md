...
The org policy forbids the Actions token from *opening* PRs, so the Action **pushes the close-out branch and prints a one-click "open PR" link in its run summary** (T-095 — the run stays green either way). **It never auto-merges** — you open (one click from the summary link) and squash-merge that one-file PR to release the lock. *(If a fine-grained PAT is ever wired in as `CLOSEOUT_PAT`, the same step opens the PR automatically — see the workflow header.)*

So the normal close-out is now: **merge your PR → open the Action's run summary → click the link → squash-merge the close-out PR.** The lock is released right after merge instead of drifting until the next task.
