...
**As of T-094 this is automated.** When a feature PR squash-merges to main, the `Post-merge close-out` GitHub Action ([.github/workflows/post-merge-closeout.yml](.github/workflows/post-merge-closeout.yml)) reads the single In Progress row, confirms its **Branch cell matches the merged PR's head branch** (so a "no ticket" override merge can't clear someone else's lock), and prepares a `chore/auto-close-tNNN` branch that moves the row to Done This Week with the squash SHA. If In Progress is empty, ambiguous (>1 row), or the branch doesn't match, the Action no-ops and warns rather than guessing.

**As of T-130 the close-out is fully autonomous when the `CLOSEOUT_PAT` repo secret is configured** (a fine-grained PAT scoped to this repo with Contents:RW + Pull requests:RW — the org policy forbids the Actions *token* from opening PRs, and the PAT bypasses that without loosening the org-wide policy). With the PAT, the Action opens the `chore/auto-close-tNNN` PR **and squash-merges it immediately** (the "Protect main" ruleset requires a PR but zero approvals), then deletes the branch. The human speed bump stays on feature PRs; the close-out PR is a machine-generated, guarded, one-row TASKS.md edit — auto-merging it is the point. Rationale: five of the six close-outs between T-124 and T-129 drifted days-to-weeks waiting on a human click (the exact gap T-094 was built to close).

**Without the PAT, the run fails RED** (T-130 — reversing T-095's stay-green choice). The close-out branch is still pushed and the error annotation + run summary carry the one-click "open PR" link; the red ✗ and its failure notification are what keep the miss from being silent. If you see a red `Post-merge close-out` run: click the link, squash-merge the one-file PR, done — and consider wiring `CLOSEOUT_PAT` so it never needs you again.

So the normal close-out is now: **merge your PR → the Action releases the lock on its own** (or fails red telling you exactly what to click).
